...
public class MyTwo extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        int a = intent.getIntExtra("num", 0);   // "num" 이라는 name 으로 넘어온 값
                                                                   //  만약 "num" 이라는 name 이 Intent 에 없으면 default 값으로 0을 설정한다.
        int b = intent.getIntExtra("num2", 0);
        int c = intent.getIntExtra("num3", 999);
        long l = intent.getLongExtra("long", 0);

        String msg = intent.getStringExtra("msg");           // 리턴값이 Object 인 경우에는  default 값을 설정할 수 없다.
        ...
        // 첫번째 액티비티로 인텐트를 날리면??
        Button btnTwoMain = findViewById(R.id.btnToMain);
        btnTwoMain.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
