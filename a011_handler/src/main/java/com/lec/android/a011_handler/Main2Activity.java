...
public class Main2Activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // 방법3
        // 메인스레드의 Handler 객체를 외부 클래스에 넘겨줌
    ...
    class BackThread1 extends Thread {
        ...
        public void run() {
            while (true) {
                ...
                /** 방법2
                 *  메인에서 생성된Handler 객체의 postXXX() 를 통해 Runnable 객체 전달
                 */
                backValue2 += 2;
                handler2.post(new Runnable() {
                    @Override
                    public void run() {     // Runnable 의 run() 에서 메인 UI 접근
                        tvBackValue2.setText("BackVlaue2 : " + backValue2);
                    } // end run()
                });
     ...
     * // Handler 객체를 생성한 스레드 만이 다른 스레드가 전송하는 Message 나 Runnable 객체를
     * // 수신할수 있다.
     ...
     * ---------------------------------------------------------------------------------------------
     * 방법2
     * Handler 만 만들고 Thread 에서 Handler 에게 Runnable 보내기
     */
